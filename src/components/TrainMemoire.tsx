...
import { senagGameQuestions, trainCategories, sortKey } from '../../data/senagGame';
...
const TrainMemoire: React.FC<TrainMemoireProps> = ({ onBack }) => {
  ...
  const drawWagons = (cfg: SetupConfig): { shuffled: GameQuestion[]; correct: GameQuestion[] } => {
    ...
    // Ordre correct = trié chronologiquement (utilise iso si dispo, sinon year)
    const correct = [...selected].sort((a, b) => sortKey(a) - sortKey(b));
