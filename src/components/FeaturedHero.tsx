...
import { lastHero, setLastHero } from "../lib/reading";

const CONTEUSE = "/images/pati-conteuse.webp";

// L'image du héros tourne : la conteuse OU une couverture de livre (au hasard,
// en évitant celle montrée juste avant). Le TEXTE, lui, reste l'accroche de marque.
function pickBackdrop(): string {
  const covers = BOOKS.filter((b) => !b.comingSoon && b.cover).map((b) => b.heroArt || b.cover);
  const pool = [CONTEUSE, ...covers];
  const last = lastHero();
  const choices = pool.length > 1 && last ? pool.filter((c) => c !== last) : pool;
  return choices[Math.floor(Math.random() * choices.length)] || CONTEUSE;
}

export default function FeaturedHero() {
  const backdrop = useMemo(() => pickBackdrop(), []);
  useEffect(() => { setLastHero(backdrop); }, [backdrop]);
  const isConteuse = backdrop === CONTEUSE;

  return (
    <section className="relative overflow-hidden bg-[#FFF6E7]">
      {/* Image de fond (desktop) : tourne à chaque visite, fondue dans le crème */}
      <div className="hidden md:block absolute inset-0 overflow-hidden" aria-hidden="true">
        <img src={backdrop} alt="" className="w-full h-full object-cover object-center"
          style={isConteuse ? { transform: "scale(1.14) translateX(7%)", transformOrigin: "center" } : { objectPosition: "right center" }} />
        <div className="absolute inset-0" style={{ background: "linear-gradient(to right, #FFF6E7 0%, #FFF6E7 30%, rgba(255,246,231,0.72) 48%, rgba(255,246,231,0) 64%)" }} />
      </div>
      ...
      {/* Image (mobile) : pleine largeur sous le texte, fondue en haut */}
