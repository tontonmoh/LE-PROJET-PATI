...
function NotifyForm() {
  ...
  const submit = async () => {
    ...
    // 1) Honeypot : un humain ne remplit jamais ce champ caché → c'est un bot.
    if (hp) { setStatus("done"); return; }
    // 2) Anti-rejeu : au plus 1 envoi toutes les 30 s depuis ce navigateur.
    try {
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-3">
      {/* Honeypot anti-bot : invisible aux humains, doit rester vide */}
