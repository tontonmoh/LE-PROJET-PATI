...
export default function SeriePointZero() {
  ...
  return (
    <div className="bg-[#FFF6E7] min-h-screen">
      ...
      <section className="text-white" style={{ background: NAVY }}>
        <div className="max-w-5xl mx-auto px-6 py-12">
          ...
          <div className="flex flex-col md:flex-row gap-8 md:items-center">
            <img src={POINT_ZERO.cover} alt={POINT_ZERO.titre} className="w-44 md:w-52 rounded-2xl shadow-kid shrink-0 self-start" />
            <div>
              ...
              <h1 className="font-display font-bold text-3xl md:text-4xl mb-3">{POINT_ZERO.titre}</h1>
              <p className="text-lg font-semibold text-white/90 mb-3">{POINT_ZERO.accroche}</p>
              <p className="text-white/75 leading-relaxed max-w-2xl">{POINT_ZERO.intro}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-5xl mx-auto px-6 py-10">
        ...
        <p className="text-sm font-semibold text-[#5a6b62] mb-6">Un éclat par tome — à lire dans l'ordre, du point zéro jusqu'aux quatre coins du monde.</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {tomes.map((t) => {
            const meta = [`Tome ${t.numero}`, t.lieu].filter(Boolean).join(" · ");
              ...
              return (
                <Link key={t.numero} to={t.to} className="card-kid group overflow-hidden">
                  <div className="relative aspect-[16/9] overflow-hidden">
                    ...
                    <span className="absolute top-2 left-2 inline-flex items-center justify-center h-6 px-2 text-[11px] font-bold text-white rounded-full shadow-kid" style={{ background: POINT_ZERO.accent }}>Tome {t.numero}</span>
                    <div className="absolute inset-x-0 bottom-0 p-2 flex justify-end opacity-0 group-hover:opacity-100 transition-opacity bg-gradient-to-t from-black/50 to-transparent">
                      <span className="inline-flex items-center gap-1 text-xs font-bold text-white px-2.5 py-1 rounded-full" style={{ background: POINT_ZERO.accent }}><BookOpen size={14} /> Lire</span>
                    </div>
                  </div>
                  <div className="p-3">
            ...
            return (
              <div key={t.numero} className="card-kid overflow-hidden">
                <div className="relative aspect-[16/9] overflow-hidden flex items-center justify-center" style={{ background: `linear-gradient(160deg, ${POINT_ZERO.accent} 0%, #0a1428 100%)` }}>
                  <span className="absolute top-3 left-0 right-0 text-center text-[9px] font-bold uppercase tracking-[0.22em] text-white/40">Le Point Zéro</span>
