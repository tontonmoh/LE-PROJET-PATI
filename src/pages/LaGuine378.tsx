...
import { MapPin, ArrowLeft, Maximize2, Minimize2, Users } from "lucide-react";
import { useRef, useState, useEffect } from "react";
import { logProgress } from "../lib/progress";

export default function LaGuine378() {
  const wrapRef = useRef<HTMLDivElement>(null);
  const frameRef = useRef<HTMLIFrameElement>(null);
  const [fs, setFs] = useState(false);

  // Config Supabase injectée au build par Netlify (comme LeDefi).
  // Absente en local => le jeu reste en localStorage. Transmise à l'iframe au chargement.
  ...
  // - chaque sous-préfecture trouvée -> progression de l'enfant actif
  // - fin de partie -> défi terminé (temps enregistré)
  useEffect(() => {
    const onMsg = (e: MessageEvent) => {
      if (frameRef.current && e.source !== frameRef.current.contentWindow) return;
      const d = e.data;
      if (d && d.type === "pati-progress" && d.kind === "sous-prefecture" && d.ref) {
        logProgress("sous_prefecture_trouvee", String(d.ref));
      }
      if (d && d.type === "pati-progress" && d.kind === "laguine378-sprint" && typeof d.seconds === "number") {
        logProgress("laguine378_fini", undefined, d.seconds);
      }
    };
    ...
    <div className="bg-[#FFF6E7]">
      <section className="max-w-6xl mx-auto px-6 pt-6 sm:pt-10">
        <div className="flex items-center justify-between gap-3 flex-wrap mb-3">
          <div className="flex items-center gap-2">
            <MapPin className="text-[#0F6E56]" size={26} />
            <h1 className="text-2xl md:text-3xl text-[#0D2B1A]">LaGuinè378 — Le défi des sous-préfectures</h1>
          </div>
        ...
        <p className="hidden sm:block text-[#3a4a42] font-semibold mb-4">Rattache chaque sous-préfecture à sa préfecture, le plus vite possible. Chaque sous-préf trouvée sort définitivement du jeu&nbsp;— objectif&nbsp;: compléter les 378.</p>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 pb-12">
        <div
          ...
          className={
            fs
              ? "fixed inset-0 z-[9999] bg-[#FFF6DD]"
              : "rounded-[1.5rem] overflow-hidden border-8 border-white shadow-kid bg-[#FFF6DD]"
          }
        >
          <iframe
            ref={frameRef}
            onLoad={sendConfig}
            src="/jeux/laguine378.html"
            title="LaGuinè378 : le défi des sous-préfectures"
            loading="lazy"
            allow="fullscreen"
        ...
        <p className="text-center text-xs text-[#8a9389] font-semibold mt-3">Astuce&nbsp;: plein écran pour un confort de tap maximum. Chaque sous-préf devinée est marquée pour toujours&nbsp;— tu ne la reverras plus.</p>
